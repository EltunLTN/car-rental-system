...
def test_get_all_and_available_cars(car_service):
    ...
    assert len(car_service.get_all_cars()) == 2
...
def test_update_car(car_service):
    ...
    assert car_service.update_car("INVALID", {}) is None
...
def test_delete_car(car_service):
    ...
    car_service.rentals_repo.create({
        "rental_id": "R1", "car": car1.to_dict(),
        "client": {"client_id": "C1", "name": "Test", "email": "a@b.com", "phone": "123"},
        "start_date": "2024-01-01", "end_date": None, "total_cost": 0.0, "is_active": True
    })
    
    assert car_service.delete_car(car1.vehicle_id) is False
    assert isinstance(car_service.delete_car(car2.vehicle_id), Car)
...
def test_calculate_rental_cost(car_service):
    ...
    with pytest.raises(Exception):
        car_service.calculate_rental_cost(car.vehicle_id, -1)
    with pytest.raises(Exception):
        car_service.calculate_rental_cost("INVALID", 3)
...
def test_search_cars(car_service):
    ...
    assert len(car_service.search_cars(car_type="Sedan")) == 1
    assert len(car_service.search_cars(brand="Toyota")) == 2
    assert len(car_service.search_cars(brand="Toyota", car_type="SUV")) == 1
    assert len(car_service.search_cars(brand="Ferrari")) == 0